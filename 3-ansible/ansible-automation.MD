...
sudo apt install ansible rpm tree vim
```
...
```
sudo apt update
sudo apt install sshpass tree rpm vim
```
...
- Enter the password as ranga12345
...
```
ansible-playbook handlers.yaml  --list-tags
```

```
ansible-playbook handlers.yaml  --tags apache2
```
...
## Step 14 : Copy files from ansible server to clients
...
## Step 15 : Copy files between the client machines using the remote_src command
